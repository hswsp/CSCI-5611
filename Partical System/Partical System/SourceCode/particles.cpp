...
//	glClearColor(0.298f, 0.796f, 1.0f, 0.6f);
	...
	do
	{
		...
		int newparticles;
#ifdef FOUNTAIN
		newparticles = (int)(delta*19000.0);
		if (newparticles > (int)(0.029f*10000.0))//fix to 35 FPS
			newparticles = (int)(0.029f*10000.0);
...
//#pragma omp parallel for num_threads(3)
		for (int i = 0; i < newparticles; i++)
		{
			int particleIndex = FindUnusedParticle();
			{
				...
				ParticlesContainer[particleIndex].a = 2 * (rand() % 256) / 3 + 85;// 65
				ParticlesContainer[particleIndex].size = (rand() % 1000) / 2500.0f + 0.06f;//rand()
...
void DrawFloor(GLuint programID, GLuint *vao, glm::mat4 view, glm::mat4 proj)//
{
	...
	model = glm::scale(model, glm::vec3(200, 0, 200));
	...
	if (CatchTeapot)
	{
		...
		float sign = dot(glm::vec3(0, 0, 1), Cameradirection);
		float angle = acos(dot(glm::vec3(1, 0, 0), Cameradirection));
		if (sign > 10e-3)
			angle = -angle;
...
bool computephysics(Particle& p,double delta)
{
	...
	{
		p.life = -1;
		const float resistFactor = 0.8;
		p.cameradistance = -1.0f;
		if (p.level == 2)
			return false;
		p.pos.y = Origin.y+ p.size;
		glm::vec3 vNorm = dot(p.speed, FloorN)*FloorN;
		p.speed -=(1+ FloorCor)*vNorm ;//* resistFactor
		/*p.r = 82;
		p.g = 59;
		p.b = 88;*/
		return true;
	...
	{//Simulate simple physics : sphere
		...
		p.speed -= (1 + spherecor)*vNorm;// * resistFactor
		/*p.r = 188;
		p.g = 108;
		p.b = 145;*/
		return true;
...
void CalculateFrameRate()
{
	...
	if (currentTime - lastframeTime > 1.0f)
	{
		...
		sprintf(strFrameRate,"Current Frames Per Second: %d, the total life particles is %d\n", int(framesPerSecond), ParticlesCount);
